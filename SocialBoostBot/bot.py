...
def init_db():
    conn = sqlite3.connect("boost_enterprise_v3.db")
    c = conn.cursor()
...
# --- হেল্পার ফাংশনস ---
def get_db():
    return sqlite3.connect("boost_enterprise_v3.db")
...
def check_and_merge_wallets(user_id):
    ...
    # বোনাস $0.50 হলে মেইন ওয়ালেটে যাবে
    if bonus_bal >= bonus_threshold:
        ...
        try:
            bot.send_message(user_id, f"🎉 **বোনাস আনলক!** আপনার `${bonus_bal:.4f} USD` বোনাস ব্যালেন্স সফলভাবে মেইন ওয়ালেটে যুক্ত করা হয়েছে।", parse_mode="Markdown")
        except:
            pass

    # রেফারেল ইনকাম ৳১২৫ হলে মেইন ওয়ালেটে যাবে
    if ref_bal_bdt >= ref_threshold_bdt:
        ...
        try:
            bot.send_message(user_id, f"💰 **রেফারেল ইনকাম আনলক!** আপনার `৳{ref_bal_bdt:.2f} BDT` রেফারেল কমিশন `${usd_to_add:.4f} USD` হিসেবে মেইন ওয়ালেটে যোগ হয়েছে।", parse_mode="Markdown")
        except:
            pass
...
# --- থ্রেডেড সুরক্ষিত মাল্টি-লেয়ার অটো-সিঙ্ক ---
...
def auto_sync_multitier(call):
    ...
    bot.send_message(call.message.chat.id, "⏳ SMMGen API থেকে ডেটা এনে সাজানো হচ্ছে, অনুগ্রহ করে ১০-১৫ সেকেন্ড অপেক্ষা করুন...")
...
def execute_admin_ops(call):
    ...
    if cmd == "adm_set_margin":
        ...
        if os.path.exists("boost_enterprise_v3.db"):
            with open("boost_enterprise_v3.db", "rb") as doc:
...
# --- ব্যাকগ্রাউন্ড অর্ডার ট্র্যাকার ইঞ্জিন ---
